...
void Fan::measureRPM(){
  ...
  countHalfRevs(current_value, last_value);
...
void Fan::countHalfRevs(int current_value_, int last_value_){
  if(current_value_ == HIGH && last_value_ == LOW){
    half_revs++;
    end_time_flanco = millis();
    time = end_time_flanco - start_time_flanco;
    start_time_flanco = millis();
  }
}

unsigned long Fan::timebetFlanco(){
  return time;
}
